/* 打印相关辅助函数
请在 #include "print_utils.hpp" 之前声明好自定义类型的 operator<< 重载函数,
这样可以确保在打印时能够正确调用对应的重载函数, 如打印自定义类型的 vector, set, map 等嵌套容器。
eg:
```cpp
std::ostream& operator<<(std::ostream& os, const MyType& obj); // 在这里声明自定义类型的打印函数
#include "print_utils.hpp" // 确保在这里包含 print_utils.hpp
std::ostream& operator<<(std::ostream& os, const MyType& obj) // 这里实现
{
    // 实现打印逻辑
    return os;
}
...
// 打印漂亮的 map
template<typename K, typename V>
inline std::string pretty_map(const std::map<K, V>& m) {
